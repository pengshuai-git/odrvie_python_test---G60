...
def set_damping_no_rebound(axis, damping_coefficient):
    """
    设置电机的阻尼效果（无回弹）。
    ...
    axis.controller.config.vel_integrator_gain = 0  # 禁用积分器，避免回弹
...
def main():
    ...
    damping_coefficient = float(input())  # 获取用户输入的阻尼系数
    set_damping_no_rebound(odrv0.axis0, damping_coefficient)  # 设置轴 0 的阻尼效果

    print("阻尼（无回弹）已设置。按 Ctrl+C 退出程序。")
    try:
        while True:
            time.sleep(1)  # 保持程序运行
