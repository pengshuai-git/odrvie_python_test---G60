...
def data_acquire():
    ...
    while not stop_flag.is_set():
        ...
        data_log.append([t, iq_measured, iq_setpoint, id_measured, id_setpoint, iq_error, id_error])
        time.sleep(0.001)
...
try:
    while True:
        ...
        if min_len > 0:
            ...
            # 判断是否达到3个周期，若达到则自动停止
            if t_arr[-1] >= 20*period:
                print('已采集3个周期，自动停止并关闭窗口...')
                break
        plt.pause(0.02)
    stop_flag.set()
    data_thread.join()
    data_dir = r'f:\00_Odrive\odrvie_python_test - G60\Read_v_x_i\data'
    os.makedirs(data_dir, exist_ok=True)
    plt.savefig(os.path.join(data_dir, 'output.png'))
    import csv
    with open(os.path.join(data_dir, 'output.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['t', 'iq_measured', 'iq_setpoint', 'id_measured', 'id_setpoint', 'iq_error', 'id_error'])
